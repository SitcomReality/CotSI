...
**Descriptor data (object geometry):** One file per object in `src/render/hexmap3d/worldObjects/descriptors/data/` — id `knot` → `knot.js` exporting `KNOT_DESCRIPTOR` (see `descriptorExportName` in `dev/geometryEditor/emitDescriptor.js`). Files are generated by the geometry editor: edit the object in `dev/geometryEditor.html`, press **Save** (needs `dev/geometryEditor/saveServer.sh`, which also serves the game). Don't hand-edit them — the next save overwrites. Parts may nest into **groups** (`children`, no `shape` — `dev/descriptorAuthoring.md` §4.5); author the tree in the editor. `base.js`/`mob.js` stay table-driven (`BASE_VARIANTS`/`MOB_VARIANTS` — mobs compose the per-archetype variant files in `descriptors/data/mobs/`) and are rejected by Save for now. The only non-descriptor geometry is the procedural fruit tree (`worldObjects/fruitTree/`). Round-trip safety is pinned in `tests/render/descriptorRoundtrip.test.js`.
...
| `dev/mobGeometryAndAnimation.md` | Mob geometry & animation design notes (joint groups, FK chains, animation runtime proposal) |
