...
Verifies that every relative import in dev/geometryEditor/ resolves to a real
...
Usage:  python3 dev/check_geometry_editor_imports.py
...
EDITOR = os.path.join(ROOT, 'dev', 'geometryEditor')
...
def walk_editor_js():
    for dirpath, _dirnames, filenames in os.walk(EDITOR):
        for name in sorted(filenames):
            if name.endswith('.js'):
                yield os.path.join('dev', 'geometryEditor', os.path.relpath(dirpath, EDITOR), name).replace(os.sep, '/')
