...
- **One name per thing.** A thing's display name is its only name. Log lines, tooltips, dispatch reports, faction traits, tests, and docs all quote the canonical name verbatim. Code identifiers may stay technical (`blessedFont`), but user-facing text never invents synonyms.
...
- **The archetype registry is the source of truth.** Display names live in the `defineArchetype(...)` `name` field (`feature_blessedFont` → "Blessed Font", `feature_knot` → "God's Knot"). Renderer descriptors and UI copy mirror these names. When a name changes, change the archetype, every quoted string, tests, and docs together — then finish with a repo-wide grep for the old name.
- **Words are owned by their strongest user.** `Verdant` is a faction, so no terrain is verdant. The early codex visual-theme word is banned outright (extinguished Aug 2026): grove trees are pure decor, and the heal-giving feature is the Blessed Font.
