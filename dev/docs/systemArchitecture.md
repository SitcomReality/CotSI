...
| `worldObjects/descriptors/gameBuilder.js` | Game-side tile → descriptor resolution (features + per-terrain decor — one decor per terrain, incl. the Painforest woods' gnarled biome variant) + assembly |
...
| `worldObjects/fruitTree/fruitTreeRecordsForTile.js` | Per-tile fruit-tree dispatch (fruit tree only — woods are descriptor data) |
