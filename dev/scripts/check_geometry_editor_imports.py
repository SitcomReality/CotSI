#!/usr/bin/env python3
"""
check_geometry_editor_imports.py — Import checker for the geometry editor (no deps).

Verifies that every relative import in dev/tools/geometryEditor/ resolves to a real
file, and that every named import refers to a symbol the target module actually
exports. The editor imports into src/ (descriptors, materials, lighting), so
this follows the same pattern as check_analysis_imports.py.

Does NOT check layer boundaries — those rules only apply to src/.

Usage:  python3 dev/scripts/check_geometry_editor_imports.py
...
EDITOR = os.path.join(ROOT, 'dev', 'tools', 'geometryEditor')
...
def walk_editor_js():
    for dirpath, _dirnames, filenames in os.walk(EDITOR):
        for name in sorted(filenames):
            if name.endswith('.js'):
                yield os.path.join('dev', 'tools', 'geometryEditor', os.path.relpath(dirpath, EDITOR), name).replace(os.sep, '/')
...
def main():
    ...
    if missing_exports or EXPORT_FAILURES:
        ...
        for e in missing_exports + sorted(EXPORT_FAILURES):
            print('  ' + e)
    if ok:
        print(f'OK — all geometry editor imports resolve, all named exports verified '
              f'({sum(1 for _ in walk_editor_js())} files checked).')
    else:
        print('FAILED')
