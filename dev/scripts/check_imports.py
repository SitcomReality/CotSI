...
Gates:
  ...
  3. Boundary report: cross-layer imports vs the layer rules in AGENTS.md
     ...
     pre-existing debt is tracked in dev/docs/futureWork.md; do not add to it.
...
Usage:  python3 dev/scripts/check_imports.py
...
# ---------------------------------------------------------------------------
# Layer rules (AGENTS.md / dev/docs/systemArchitecture.md §2). Layers not listed
# may import anything.
# ---------------------------------------------------------------------------
ALLOWED = {
    'shared':     {'shared'},
...
# layer — matches the tolerance policy in dev/docs/systemArchitecture.md §6.
READONLY_RULES_DATA = {
...
# mutation (tolerance policy dev/docs/systemArchitecture.md §6). Symbol-scoped —
