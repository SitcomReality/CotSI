 ...
 * (dev/tests/render/fixtures/descriptorData.snap.json) — the descriptor→record
...
test('data covers every FEATURE_VISUALS kind plus decor/mountain/knot content', () => {
  ...
  const expectedSimple = [
    'treasureChest', 'bush', 'palimpsestSlab', 'errataSlip', 'volvelle',
...
test('simple features: one record, legacy scatter bounds', () => {
  for (const raw of ALL_DESCRIPTORS) {
    ...
    // [0.8, 0.99] × the authored `size` range (size.min/size.max). The part's
    // own per-axis scale folds into `scale` (a flattened spheroid like
    // listenerLichen's mat legitimately exceeds descriptor.scale alone).
