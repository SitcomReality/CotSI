...
// ── Motif panel (v6 decor composition, decorComposition.md §6.1) ────────────
...
/**
 * The motif panel for `kind: 'decor'` descriptors with `motifs` — the v6
 * replacement for the Variant section: a motif list with weight inputs
 * (＋ Add / duplicate / delete), a per-biome grid showing each motif's
 * REALIZED share w_i/Σw (with a bar; excluded cells struck through), and a
 * "Force motif" picker that drives S.variantId (motif ids only — the record
 * path treats the explicit id as a forced motif, pin > force > weights).
 */
function renderMotifControls(container, ctx) {
