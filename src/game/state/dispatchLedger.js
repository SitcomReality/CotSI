 ...
 * dispatch report at beginTurn. Each entry is { text, sign, type } with
 * sign: 'gain' | 'loss' | 'neutral' (used for ledger line tones),
 * type: resource category for icon selection.
 */
...
export function recordLedgerEntry(champ, text, sign = 'neutral', type = 'info') {
  ...
  champ.dispatchLedger.push({ text, sign, type });
}
 ...
 *
 * @param {Object} champ
 * @returns {Array<{text: string, sign: string, type: string}>}
 */
