...
const RING = {
  ...
  transform: { y: 0.85, localAxis: { x: 1, y: 0, z: 0 }, localAngle: Math.PI / 2 },
  color: 'factionAccent',
};
...
const HANG_SPIKE = {
  ...
  transform: { y: 0.07, localAxis: { x: 1, y: 0, z: 0 }, localAngle: Math.PI },
  color: 'factionAccent',
};

/** Every faction variant. Variant id === the faction short name. */
export const BASE_VARIANTS = {
  CRU: [TOWER, CAP, ...spikes('spike', 6, 0.15, 0.28, 0.06, 0.1)],
  REV: [TOWER, CAP, RING],
  VER: [TOWER, CAP, ...spikes('crown', 8, 0.8, 0.28, 0.04, 0.08)],
  ARC: [TOWER, CAP, ...dots('dot', 4, 0.55, 0.32)],
  HRT: [TOWER, CAP, DOME],
  MSK: [TOWER, CAP, SPIRE],
