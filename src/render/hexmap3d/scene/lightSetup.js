...
/**
 * Apply the shadowLightConfig shadow parameters to a directional light.
 * Called at setup and again when shadows are toggled back on at runtime
 * (the light may never have been configured if it started disabled).
 * @param {THREE.DirectionalLight} dirLight
 */
export function applyShadowConfig(dirLight) {
  const cfg = shadowLightConfig;
