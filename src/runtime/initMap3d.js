...
import { waterCenterUniform } from '../render/hexmap3d/scene/materials.js';
...
export function initMap3D(mountEl, gameState) {
  ...
  try {
    ...
    if (gameState.radius) {
      // Water swell rolls toward the map center (always world origin), so the
      // shader only needs that center point.
